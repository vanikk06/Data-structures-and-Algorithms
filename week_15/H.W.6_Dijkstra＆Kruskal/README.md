...
- 可學習之處
  ...
  - disjoint set使用：僅用一個解析式list就完成了（**動態的list**），在disjoint set中原本每個元素都是互相獨立的，但因起點與終點的關係，會將終點放入與起點相同的set的，一步步執行，最終所有元素都會在同一個set內，而disjoint set要做的，就是將兩個不同的set進行合併，合併時，必須將被合併的set中的所有元素，更改為與合併set相同的
